        ...
        <TouchableOpacity
          ...
          <View style={styles.tabImageContainer}>
            {user._id === auth.id ? (
              ...
              <FontAwesome name="user" style={{fontSize: 40, color: '#000'}} />
            )}
          </View>
          {user._id === auth.id ? (
            <Text style={styles.tabText}>Edit Profile</Text>
          ) : (
            <Text style={styles.tabText}>View Profile</Text>
          )}
        </TouchableOpacity>

        {/* Message button - only shown if not the user's own profile */}
        {user._id !== auth.id && (
          <TouchableOpacity
            style={styles.tabContainer}
            onPress={() => {
